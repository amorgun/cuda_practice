...
)
{
    ...
    for (size_t ki = 0; ki < kernel_size; ++ki) {
        ...
        for (size_t kj = 0; kj < kernel_size; ++kj) {
            next_state_val += kernel_const[ki_offset + kj] * state_shared[src_i_offset + kj];
        }
    }
...
int main(int argc, char *argv[])
{
    ...
    for (size_t it = 0; it < nsteps; ++it) {
        ...
        make_step<<<grid_shape, block_shape, (block_shape.x+2*R)*(block_shape.y+2*R)*sizeof(int), kernel_stream>>>(current_field_gpu, next_field_gpu, FIELD_SIZE, KERNEL_SIZE, dT, GROWSH_MEAN, GROWSH_STD);
