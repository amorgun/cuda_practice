...
)
{
    ...
    for (size_t ki = 0; ki < kernel_size; ++ki) {
        ...
        for (size_t kj = 0; kj < kernel_size; ++kj) {
            ...
            for (size_t i = 0, dst_i = (i + size + R - ki) % size; i < size; ++i, ++dst_i) {
                dst_i -= (dst_i > size) ? size : 0;
                const size_t i_offset = i * size;
                const size_t dst_i_offset = dst_i * size;
                for (size_t j = 0, dst_j = (j + size + R - kj) % size; j < size; ++j, ++dst_j) {
                    dst_j -= (dst_j > size) ? size : 0;
                    next_state[dst_i_offset + dst_j] += k * current_state[i_offset + j];
                }
            }
        }
    }
