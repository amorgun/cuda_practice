...
int main(int argc, char *argv[])
{
    ...
    for (size_t it = 0; it < nsteps; ++it) {
        ...
        pad_field(current_field_gpu, current_field_padded_gpu, FIELD_SIZE, R);
        make_step<<<grid_shape, blocks_shape>>>(current_field_gpu, current_field_padded_gpu, next_field_gpu, FIELD_SIZE, kernel_gpu, KERNEL_SIZE, dT, GROWSH_MEAN, GROWSH_STD);
